class RemindersController < ApplicationController
  ...
  def test_email
    plugin_settings = resolve_plugin_settings
  ...
  def reset_template
    current_settings = resolve_plugin_settings
  ...
  def resolve_plugin_settings
    ...
    if raw_settings.key?('plugin') && raw_settings['plugin'].is_a?(Hash)
      raw_settings['plugin']
    else
      raw_settings
    end
  ...
  def save_settings
    ...
    # Clean up any previously saved nested format {"plugin" => {...}}
    # by saving the flat format directly
